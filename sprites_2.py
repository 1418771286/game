...
class Background(GameSprite):
    ...
    def __init__(self, is_alt = False):
        ...
        super().__init__("./images/background7.png", speed=[0, 1])

        #实现一张图片颠倒连接
        if is_alt:
            image = pygame.image.load("./images/background7.png")
...
class PlaneSprite(GameSprite):
    ...
    def update(self):
        """实现生存和爆炸图片连续播放"""
        super().update()
        index1 = int(self.show_image_index)  #可以把小数去掉
        ...
        #当索引到最后一张图片，可以摧毁（索引是从0开始的）
        elif self.show_image_index > count - 1:
            self.show_image_index = count - 1
            self.can_destroied = True

        index2 = int(self.show_image_index)  #把小数去掉
        if index1 != index2:
            #image在GameSprite类的初始方法中切换图片
...
class Enemy(PlaneSprite):
    ...
    def __init__(self):
        ...
        super().__init__(image_names, destroy_names, ENEMY_LIFE)
        #随机敌机出现的位置和速度
        width = SCREEN_RECT.width - self.rect.width
        self.rect.left = random.randint(0, width)
        self.rect.bottom = 0
        self.speed = [0, random.uniform(1, 3)]
